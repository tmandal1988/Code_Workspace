...
const char *AppName = "GenVI Avionics Netburner Blue v1";

static uint16_t pilot_input[4] = { 0 }; //0.Throttle, 1.Aileron, 2.Elevator,3.Rudder
static uint8_t GPS_ms[4] = { 0 }; //Array to store GPS time (ms from the beginning of the GPS week)
...
void Read_GPS_Data(void*) { //Reads Data from GPS
	...
	while (1) {
		...
		GPS_ms[0] = GPS_packet[16];
		GPS_ms[1] = GPS_packet[17];
		GPS_ms[2] = GPS_packet[18];
		GPS_ms[3] = GPS_packet[19];
	...
	while (J1[7]) {
		...
		if (status == OS_NO_ERR) {
			...
			while (!ADDone())
				...
				asm("nop");
			elevator_pot = ((double) (GetADResult(1))) * 3.3 / 32768.0;

			//Display_Roll(complimentary_filter.roll);//Displays roll value, argument roll
			//Display_Pitch(complimentary_filter.pitch);//Display pitch values, argument pitch
			//Replace_Center_Line(complimentary_filter.pitch,complimentary_filter.roll);//deletes old center line and redraws it based on new roll and pitch value, first argument pitch, second argument roll
